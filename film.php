...
if (isset($_GET['id'])) {
  $top = $pdo->query("SELECT * FROM film WHERE id =" . $_GET['id']);
  foreach ($top as $row) {
?>
<img class="prof" src="profile.png" alt="profiel" width="70"> 
    ...
    $count = $pdo->query("SELECT count(film_id) from favoriet where user_id = " . $gebruiker . " AND film_id = " . $_GET['id'] . "")->fetchColumn();
  
    if ($count == 0){
    ?>
      <a href="fav.php"><img src="heart.png" width="50"></a>
    <?php
    } else {
    ?>
      <a href="fav.php"><img src="hart.png" width="50"></a>
    <?php
    }
    ?>
      ...
      ?>
        <div class="info">
          ...
          <?php
            ...
            echo "<tr><td>Rating</td><td>" . $row['score'] . "</td></tr>";
            echo "<tr><td>Release datum</td><td>" . $row['datum'] . "</td></tr>";
            echo "<tr><td>Duur</td><td>" . $row['duur'] . "</td></tr>";
          ?>
          </table>
        </div>
    </div>
