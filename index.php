    ...
    <div class="flex-container">
      <div class="favorietfilm">
        ...
        <table>
          ...
          try {
            ...
            $top = $pdo1->query("SELECT id, titel, score, foto, type FROM film 
              ORDER BY score DESC LIMIT 0, 6");
            foreach ($top as $row) {
              if ($row["type"] === "normaal") {
                echo "<tr><td><img width=100 src=" . "" . $row['foto'] . "></td><td>" . $row['titel'] . "</td><td>" . $row['score'] . "</td><td><a href='film.php?id=" . $row['id'] . "'>></a></td></tr>";
      ...
      <div class="favorietserie">
        <table>
          ...
          <?php
            try {
              $pdo2 = new PDO("mysql:host=localhost;dbname=Fresh_tomatoes", 'bit_academy', 'bit_academy');
              $pdo2->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            } catch (PDOException $e) {
              echo "Connection failed: " . $e->getMessage();
            }
       
            $top = $pdo2->query("SELECT id, titel, score, foto, type FROM film 
              ORDER BY score DESC LIMIT 0, 15");
            foreach ($top as $row) {
              if ($row["type"] === "netflix") {
      ...
      <div class="komenuit">
        <table>
          ...
          <?php
            try {
              $pdo3 = new PDO("mysql:host=localhost;dbname=Fresh_tomatoes", 'bit_academy', 'bit_academy');
              $pdo3->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            } catch (PDOException $e) {
              echo "Connection failed: " . $e->getMessage();
            }

            $top = $pdo3->query("SELECT id, titel, foto FROM fverwacht LIMIT 0, 5");
            foreach ($top as $row) {
              echo  "<tr><td><img width=100 src=" . "" . $row['foto'] . "></td><td>" . $row['titel'] . "</td><td><a href='fverwacht.php?id=" . $row['id'] . "'>></a></td></tr>";   
            } 
          ?>
        </table>
      </div>  
