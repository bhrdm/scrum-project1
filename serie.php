...
if (isset($_GET['id'])) {
  $top = $pdo->query("SELECT * FROM serie WHERE id =" . $_GET['id']);
  foreach ($top as $row) {
?>
<img class="prof" src="profile.png" alt="profiel" width="70"> 
    ...
    $count = $pdo->query("SELECT count(serie_id) from favoriet where user_id = " . $gebruiker . " AND serie_id = " . $_GET['id'] . "")->fetchColumn();
      ...
      ?>
        <div class="info">
          ...
          <?php
            ...
            echo "<tr><td>Seizoenen</td><td>" . $row['seizoen'] . "</td></tr>";
          ?>
          </table>
        </div>
